...
from ...layers.weight_init import kaiming_init
...
class GANetHead(nn.Module):
    def __init__(self,
        ...
        self.loss_kp_offset = loss_kp_offset
        self.loss_sp_offset = loss_sp_offset
...
#        print(x.keys())
        ...
        if self.training:
            output['deform_points'] = x['deform_points'][0]
    ...
    def loss(self,
        ...
        gt_kpts_hm = gt_targets['gt_kpts_hm'].to(kpts_hm.device)
        gt_kp_offset = gt_targets['gt_kp_offset'].to(kpts_hm.device)
    ...
    def get_lanes(self, preds_dicts, cfg=None):
        ...
        results_list = []
        for kpts_hm, kp_offset, sp_offset in zip(output_kpts_hms, output_kp_offsets, output_sp_offsets,
                                                           ):
            # kpts_hm: (C=1, H3, W3)
            # kp_offset: (2, H3, W3)
            # sp_offset: (2, H3, W3)
            ...
            pred = self.predictions_to_pred(lanes, groups_sps)
    ...
    def predictions_to_pred(self, lanes, group_sps):
        '''
        ...
        '''
        ori_height, ori_width = self.cfg.ori_img_h,self.cfg.ori_img_w
        img_height, img_width = self.cfg.img_h,self.cfg.img_w
