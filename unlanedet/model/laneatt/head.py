...
from ..module.architecture.detector import move_device_like
...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        # Move to cuda device
        self.anchors = move_device_like(self.anchors,self.device_tensor)
        self.anchor_ys = move_device_like(self.anchor_ys,self.device_tensor)
        self.cut_zs = move_device_like(self.cut_zs,self.device_tensor)
        self.cut_ys = move_device_like(self.cut_ys,self.device_tensor)
        self.cut_xs = move_device_like(self.cut_xs,self.device_tensor)
        self.invalid_mask = move_device_like(self.invalid_mask,self.device_tensor)
    ...
    def loss(self, output, batch, cls_loss_weight=10):
        ...
        return {'cls_loss': cls_loss_weight * cls_loss, 'reg_loss': reg_loss}
