import torch
from ..CLRNet.line_iou import line_iou
...
def dynamic_k_assign(cost, pair_wise_ious):
    ...
    for gt_idx in range(num_gt):
        # TODO has been modified to one 2 one pair
...
def assign(
    ...
    # TODO theta is not unified
    target_thetas = targets[:, 4].unsqueeze(-1)/180
   
    theta_score = torch.cdist(predictions[:, 4].unsqueeze(-1),
                              target_thetas,
                              p=1).reshape(num_priors, num_targets)
    ...
    #* only start points counted
    # cost = -start_xys_score * theta_score * distances_score
