...
from .Glineiou_loss import gliou_loss as liou_loss
...
from .tlan_CNN import CNNhead
from .tlan_FlattenMHA import MHA
from .TLAN_head import TLAN
from ..module.head import PlainDecoder

class GSEHead(nn.Module):
    def __init__(self,
                 num_points=72,
        ...
        self.TLAN = TLAN()
        self.TLANlinear1 = nn.Linear(2560,1920)
        self.TLANlinear2 = nn.Linear(2560,1920)
        
        self.linear2 = nn.Linear(74,64)
        self.linear3 = nn.Linear(74,64)
        self.ViTdim = 74
    ...
    def forward(self, x,x2, **kwargs):
        ...
        for stage in range(self.refine_layers):
            ...
            TLANfea1,TLANfea2 = self.TLAN(x2)
            TLANfea1 = self.TLANlinear1(TLANfea1)
            TLANfea2 = self.TLANlinear2(TLANfea2)
            TLANfea1 = TLANfea1.reshape(batch_size * num_priors,10)
            TLANfea2 = TLANfea2.reshape(batch_size * num_priors,10)

            cls_features = fc_features.clone()
            cls_features = torch.cat((cls_features,TLANfea2),dim = 1)
            cls_features = self.linear2(cls_features)


            reg_features = fc_features.clone()
            reg_features = torch.cat((reg_features,TLANfea1),dim = 1)
            reg_features = self.linear3(reg_features)
        ...
        for stage in range(self.refine_layers):
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                iou_loss = iou_loss + liou_loss(
                    ...
                    self.img_w,self.img_h,length=15)
...
#        import pdb;pdb.set_trace()
        ...
        angle_loss = angle_loss_weight * angle_loss
        alpha = 0.98
        iou_loss = alpha * iou_loss + (1 - alpha) * angle_loss
        ...
        return_value = {
                ...
                'giou_loss': iou_loss * iou_loss_weight
            }
