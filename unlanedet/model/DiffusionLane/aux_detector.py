    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            if self.aux_head is not None:
                aux_feat = self.aux_head(fea)
                loss_aux = self.aux_head.loss(aux_feat,batch=batch)
                for k, v in loss_aux.items():
                    k = k + "_aux_clr"
                    output[k] = self.aux_weight*v
        else:
        ...
        if self.training:
            ...
            priors[..., 6:] = (
                                      ...
                                      ((1 - self.prior_ys.repeat(num_priors * batch, 1).view(batch, num_priors, -1) -
                                       ...
                                       self.head.img_h / torch.tan(priors[..., 4].unsqueeze(2).clone().repeat(1, 1,
                                                                                                              self.n_offsets) * math.pi + 1e-5))) / (
                                          self.head.img_w - 1)
    ...
    def renewal_batch_lane(self,
        ...
        for scores_per_img, pred_noise_per_img, x_start_per_img, per_img in zip(scores, pred_noise, x_start, img):
            ...
            if self.num_proposals - num_remain > 0:
                per_img = torch.cat(
                    (per_img, self.generate_priors_from_embeddings(self.num_proposals - num_remain, batch=1)[0]), dim=0)
